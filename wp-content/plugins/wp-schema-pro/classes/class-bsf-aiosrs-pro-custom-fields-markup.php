...
if ( ! class_exists( 'BSF_AIOSRS_Pro_Custom_Fields_Markup' ) ) {
	...
	class BSF_AIOSRS_Pro_Custom_Fields_Markup {
		...
		public function setup_meta_box() {
			$brand_settings = isset(BSF_AIOSRS_Pro_Helper::$settings['wp-schema-pro-branding-settings']) ? BSF_AIOSRS_Pro_Helper::$settings['wp-schema-pro-branding-settings'] : [];
		
			$this->init_static_fields();
			if ( ! empty( self::$meta_boxes ) ) {
				$plugin_name = isset($brand_settings['sp_plugin_name']) ? $brand_settings['sp_plugin_name'] : '';
				if ( '' !== $plugin_name ) {
					$title = __( $plugin_name, 'wp-schema-pro' ); //phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
				} else {
