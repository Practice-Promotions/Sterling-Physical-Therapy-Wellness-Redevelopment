	...
	class BSF_AIOSRS_Pro_Schema_Course {
		...
		public static function render( $data, $post ) {
			...
			if ( isset( $data['reviews'] ) && is_array( $data['reviews'] ) && ! empty( $data['reviews'] ) ) {
				foreach ( $data['reviews'] as $review_key => $review_value ) {
					if ( ! empty( $review_value['author-name'] ) || ! empty( $review_value['date-published'] ) || ! empty( $review_value['rating-value'] ) ) {
						$schema['review'][ $review_key ]['@type']                       = 'Review';
						$schema['review'][ $review_key ]['author']['@type']             = 'Person';
						$schema['review'][ $review_key ]['author']['name']              = ! empty( $review_value['author-name'] ) ? wp_strip_all_tags( (string) $review_value['author-name'] ) : null;
						$schema['review'][ $review_key ]['datePublished']               = ! empty( $review_value['date-published'] ) ? wp_strip_all_tags( (string) $review_value['date-published'] ) : null;
						$schema['review'][ $review_key ]['reviewRating']['@type']       = 'Rating';
						$schema['review'][ $review_key ]['reviewRating']['ratingValue'] = ! empty( $review_value['rating-value'] ) ? wp_strip_all_tags( (string) $review_value['rating-value'] ) : null;
					}
				}
			}
